...
{
	auto mods = modegen::parse("module mod v1.0: #help mes\nmodule mod2 v1.0:");
	BOOST_REQUIRE_EQUAL(mods.size(), 2);
	std::string result = modegen::converters::to_json(mods);
	BOOST_CHECK_EQUAL(result, R"-([{"name":"mod","type":"module","v":{"major":1,"minor":0}},{"docs":"help mes","name":"mod2","type":"module","v":{"major":1,"minor":0}}])-");
}
...
{
	auto mods = modegen::parse("module mod v1.0: enum some{one two};");
	BOOST_REQUIRE_EQUAL(mods.size(), 1);
	cppjson::value result = modegen::converters::to_json(mods);
...
{
	auto mods = modegen::parse("module mod v1.2: record rec { type m1; list<other_type> m2;}");
	BOOST_REQUIRE_EQUAL(mods.size(), 1);
	cppjson::value result = modegen::converters::to_json(mods);
	BOOST_CHECK_EQUAL(result[0]["name"], "mod");
	BOOST_CHECK_EQUAL(result[0]["v"]["major"], 1);
...
{
	auto mods = modegen::parse("module mod v1.0: interface i { type name() mutable ; constructor(); constructor(type name); }");
	BOOST_REQUIRE_EQUAL(mods.size(), 1);
